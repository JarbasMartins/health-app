        ...
        <div className="w-full h-auto space-y-4">
            ...
            <Card icon={Pill} className="h-auto">
                <div className="flex flex-col items-center text-center justify-center py-8">
                    ...
                    <h2 className="text-base md:text-lg lg:text-xl font-normal opacity-60">Nenhum medicamento cadastrado</h2>
                    <p className="text-sm md:text-base lg:text-lg opacity-60 px-4">Clique em adicionar para cadastrar seu medicamento</p>
                </div>
            </Card>
            ...
            <Modal isOpen={open} onClose={() => setOpen(false)}>
                <FormContainer
                    ...
                    footer={
                        <div className="flex justify-end gap-4 pt-4">
                            <button type="button" onClick={() => setOpen(false)} className="px-4 py-2 rounded-lg bg-gray-300 hover:bg-gray-400">
                                Cancelar
                            </button>
                ...
                >
                    <div>
                        ...
                        <input {...register('name')} className="w-full border px-3 py-2 rounded mt-1" placeholder="Ex: Ibuprofeno 600mg" />
                        {errors.name && <p className="text-red-500 text-sm">{errors.name.message}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium">Horário</label>
                        <input type="datetime-local" {...register('time')} className="w-full border px-3 py-2 rounded mt-1" />
                        {errors.time && <p className="text-red-500 text-sm">{errors.time.message}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium">Quantidade</label>
                        <input placeholder="Diga a quantidade..." {...register('quantity')} className="w-full border px-3 py-2 rounded mt-1" min={1} />
                        {errors.quantity && <p className="text-red-500 text-sm">{errors.quantity.message}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium">Descrição (opcional)</label>
                        <textarea placeholder="Adicione um descrição..." {...register('description')} rows={3} className="w-full border px-3 py-2 rounded mt-1" />
                    </div>
                </FormContainer>
            </Modal>
        </div>
